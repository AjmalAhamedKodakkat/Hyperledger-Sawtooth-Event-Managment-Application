...
router.get('/', function (req, res, next) {
  res.render('Home', { title: 'Event Management Application' });
});
 ...
 * @dev  Function to register the details of the Event 
 */
router.post('/registration', async function (req, res) {
  ...
  if (clientExist == 1) {
    res.send({ message: "Registration of Event on " + data3 + " done and  Data successfully added" });
  }
...
/**
 * @title result
 * @dev  Function to view the details of the Event's 
 */
...
router.post('/delete', async function (req, res) {
  ...
  if (clientExist == 1) {
    res.send({ message: "Event on " + data2 + " deleted successfully " });
  }
  else if (clientExist == 0) {
